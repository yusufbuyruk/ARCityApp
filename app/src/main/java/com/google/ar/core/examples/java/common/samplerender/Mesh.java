...
public class Mesh implements Closeable {
  ...
  public static Mesh createCubeMesh(SampleRender render) {
    float[] vertices = {
            0.0f, 0.0f, 0.0f, // 0
            1.0f, 0.0f, 0.0f, // 1
            1.0f, 1.0f, 0.0f, // 2
    ...
    // FloatBuffer
    ByteBuffer vbBytes = ByteBuffer.allocateDirect(vertices.length * Float.BYTES);
    vbBytes.order(ByteOrder.nativeOrder());
    FloatBuffer vertexBufferData = vbBytes.asFloatBuffer();
    vertexBufferData.put(vertices);
    vertexBufferData.position(0);

    ByteBuffer nbBytes = ByteBuffer.allocateDirect(normals.length * Float.BYTES);
    nbBytes.order(ByteOrder.nativeOrder());
    FloatBuffer normalBufferData = nbBytes.asFloatBuffer();
    normalBufferData.put(normals);
    normalBufferData.position(0);

    // IntBuffer
    ByteBuffer ibBytes = ByteBuffer.allocateDirect(indices.length * Integer.BYTES);
    ibBytes.order(ByteOrder.nativeOrder());
    IntBuffer indexBufferData = ibBytes.asIntBuffer();
    indexBufferData.put(indices);
    indexBufferData.position(0);

    VertexBuffer vertexBuffer = new VertexBuffer(render, 3, vertexBufferData);
    VertexBuffer normalBuffer = new VertexBuffer(render, 3, normalBufferData);
    IndexBuffer indexBuffer = new IndexBuffer(render, indexBufferData);

    return new Mesh(render,Mesh.PrimitiveMode.TRIANGLES, indexBuffer, new VertexBuffer[] {vertexBuffer, normalBuffer});
  }
